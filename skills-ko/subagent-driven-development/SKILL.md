...
작성된 구현 계획을 실행할 때, 순차적으로 각 작업당 신규 하위 에이전트를 하나씩 파견(dispatch)하여 작업을 수행하고, 전체 구현 완료 후 최종 리뷰를 한 번 수행합니다.
...
**핵심 원칙:** 작업당 신규 하위 에이전트 + 마지막에 최종 리뷰 = 한 번의 품질 체크포인트로 빠른 반복 수행
...
digraph process {
    ...
    "전체 구현에 대해 최종 코드 리뷰어 하위 에이전트 파견" [shape=box];
    ...
    "남은 작업이 있습니까?" -> "전체 구현에 대해 최종 코드 리뷰어 하위 에이전트 파견" [label="아니오"];
    "전체 구현에 대해 최종 코드 리뷰어 하위 에이전트 파견" -> "명시적인 통합 요청이 있습니까?";
...
**완료 (DONE):** 다음 작업으로 넘어가거나, 모든 작업이 완료된 경우 최종 리뷰로 넘어갑니다.
...
- 최종 리뷰에서 머지(merge)전에 다수 작업 간에 걸친 이슈들을 찾아냅니다.
...
- 최종 리뷰에서 나중에 이슈를 찾아낼 수는 있지만 반복 속도는 향상됩니다.
...
- 변경의 위험도가 큰 작업인데도 최종 리뷰를 생략하고 구현자의 '자체 검토'로 갈음하기
...
- **superpowers:requesting-code-review** - 리뷰어 하위 에이전트를 위한 코드 검토 템플릿입니다.
