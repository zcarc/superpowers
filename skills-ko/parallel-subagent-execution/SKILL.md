...
### 4단계: 대기, 리뷰 및 웨이브 통합 (Wait, Review, and Integrate the Wave)
...
만약 어떤 하위 에이전트가 `BLOCKED` 또는 `NEEDS_CONTEXT` 상태를 리포트한다면, 다음 웨이브를 발송하기 전에 멈추고 이를 해결하십시오.
...
### 6단계: 최종 검증 및 리뷰 (Final Verification and Review)

모든 웨이브가 완료된 후:
- 관련된 전체 단위 테스트를 실행합니다.
- `superpowers:requesting-code-review`를 호출합니다.
- '중요(Important)' 이나 '심각(Critical)' 등의 문제를 수정합니다.
...
- **superpowers:writing-plans** - 이 스킬이 실행하게 될 계획표 작성
- **superpowers:requesting-code-review** - 전체 웨이브가 완전히 끝난 뒤 합쳐진 결과를 리뷰
