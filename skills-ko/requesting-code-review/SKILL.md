...
description: 사용자가 리뷰를 요청하거나 완료된 변경 사항이 통합 전 리뷰-리스크 임계값을 넘을 때 사용
---

# 코드 리뷰 요청 (Requesting Code Review)

머지나 핸드오프 전에 문제를 포착하기 위해 `superpowers:code-reviewer` 서브에이전트를 파견하십시오. 리뷰어에게는 평가를 위한 정밀하게 구성된 문맥만 제공되며, 여러분의 세션 히스토리는 전달되지 않습니다. 이는 리뷰어가 여러분의 사고 과정이 아닌 결과물에만 집중하게 하며, 여러분은 자신의 문맥을 유지하며 작업을 계속할 수 있게 해줍니다.
...
- 병렬 서브에이전트 실행(parallel subagent execution)의 모든 웨이브(wave) 완료 후
- 주요 기능 구현 완료 후
- main 브랜치로 머지하기 전
...
다음 중 하나 이상에 해당하면 리뷰를 요청하십시오:
- 사용자가 명시적으로 리뷰를 요청할 때
- 5개 이상의 파일이 변경되었을 때
- 영속성(persistence), 스키마 또는 저장소 로직이 변경되었을 때
- 공유 추상화 또는 아키텍처가 변경되었을 때
- 중대한 통합 또는 회귀(regression) 리스크가 있을 때

다음의 경우 기본적으로 리뷰를 요청하지 마십시오:
- 소규모의 로컬 버그 수정
- 격리된 디스플레이 또는 포맷팅 변경
- 문구 수정(copy-only changes)만 있는 경우

## 요청 방법

**1. git SHA 가져오기:**
```bash
# 기본값: 전체 브랜치 또는 완료된 작업 시리즈 리뷰
BASE_SHA=$(git merge-base HEAD origin/main)  # 또는 origin/master / 실제 베이스 브랜치
HEAD_SHA=$(git rev-parse HEAD)

# 단일 커밋의 부분 체크만 필요한 경우:
...
BASE_SHA=$(git merge-base HEAD origin/main)  # 또는 실제 베이스 브랜치 / 작업 시리즈의 시작 SHA
HEAD_SHA=$(git rev-parse HEAD)
...
**인라인 실행 (Executing Plans):**
- 인라인 실행에서 리뷰는 선택 사항이며 리스크에 기반합니다.
- 변경 사항이 위험하거나 여러 작업에 걸쳐 있는 경우 주요 배치 후에 리뷰를 요청하십시오.
- 완료된 변경 세트가 비중이 큰 경우 머지 전에 항상 리뷰를 요청하십시오.

**병렬 서브에이전트 실행 (Parallel Subagent Execution):**
- 모든 웨이브(wave)가 통합된 후 한 번 리뷰를 요청하십시오.
- 브랜치를 마무리하기 전에 웨이브 간 통합 이슈를 포착하십시오.
- 머지하거나 브랜치를 완료하기 전에 중요한 이슈를 수정하십시오.
