...
description: 통합 또는 핸드오프 전에 완료된 전체 통합 변경 사항 세트에 대한 공식 리뷰가 필요할 때 사용합니다.
---

# 코드 리뷰 요청 (Requesting Code Review)

머지 또는 핸드오프 전 완료된 전체 통합 결과물에서 이슈를 잡기 위해 `superpowers:code-reviewer` 하위 에이전트를 발송합니다. 리뷰어는 평가를 위해 정밀하게 작성된 컨텍스트를 제공받으며, 여러분의 세션 히스토리는 절대 전달되지 않습니다. 이는 리뷰어가 여러분의 사고 과정이 아닌 작업 결과물에 집중하게 하고, 여러분은 통합 작업을 위해 자신의 컨텍스트를 유지할 수 있게 합니다.
...
**필수:**
- 플랜 실행(executing-plans)의 모든 작업 완료 후
...
- 표준 워크플로우 타이밍: 모든 구현 작업이 완료되고 통합된 결과가 검증된 후 한 번 공식 리뷰를 요청하십시오.
- 통합 후 완료된 브랜치 또는 작업 시리즈 전체를 리뷰하십시오. 일상적인 작업 단위 체크포인트가 아닙니다.
- 리뷰가 기술적으로 평가해야 할 피드백을 반환하는 경우에만 `superpowers:receiving-code-review`를 사용하십시오.
...
- 모든 작업이 완료된 후 한 번 리뷰하십시오.
- 개별 작업 단위가 아닌, 완료된 전체 통합 결과물을 리뷰하십시오.
- 머지 또는 브랜치 완료 전에 중요(Important) 이슈를 수정하십시오.
...
**절대 금지:**
- "간단하니까"라는 이유로 워크플로우에 필수적인 최종 공식 리뷰 건너뛰기
- 치명적(Critical) 문제 무시
